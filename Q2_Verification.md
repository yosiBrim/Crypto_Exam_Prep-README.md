...
### מה זה בעצם Stub ולמה צריך אותו?
כאשר אין לנו את קוד המקור של רכיב מסוים (כי הוא IP קופסה שחורה, או שעדיין לא פיתחנו אותו), נייצר עבורו **מודול דמה (Stub)**. 
תפקידיו המרכזיים של ה-Stub:
1. **חותמת זהות:** מכיל את אותן כניסות ויציאות בדיוק כמו הרכיב האמיתי, מה שמאפשר קומפילציה של המערכת כולה ללא שגיאות.
2. **חיקוי התנהגות (Timing):** מדמה במדויק את זמני ההשהיה (למשל ספירת 64 מחזורי שעון) ואת פרוטוקול לחיצת היד (הורדת והעלאת דגל `ready`), כדי לבדוק שהמערכת העוטפת יודעת להמתין.
3. **התעלמות ובידוד תקלות:** ה-Stub לרוב מתעלם מנתוני הקלט (`data_in`). במקום לבצע חישוב מורכב, הוא שולף "תשובות זהב" ידועות מראש מקובץ או ממשפט `case`. זה מבודד את הבדיקה למערכת העוטפת, ומבטיח שהתשובות שחוזרות הן נכונות ב-100%.
...
* **עץ ההחלטות (איזו שבלונה לבחור במבחן?):**
  * **האם יש ל-DUT סיגנלי `start` ו-`ready`?**
    * **כן:** המערכת סדרתית ולוקח לה זמן לחשב. **-> בחר בשבלונה 4ב (עם FSM).**
    * **לא:** המערכת קומבינטורית (כמו SBOX או מודול Loop Unrolled). **-> בחר בשבלונה 4א (קריאה רציפה).**

---

## 4א. שבלונת Testbench למערכת קומבינטורית (ללא start/ready)
...
## 4ב. שבלונת Testbench למערכת סדרתית (מודל FSM של שטרו)
...
## 5. התבנית האולטימטיבית ל-Stub עם השהיות אלגוריתמיות (File I/O)
